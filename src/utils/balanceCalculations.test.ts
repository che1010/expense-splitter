...
describe('computeBalances', () => {
  ...
  it('does NOT reduce debt when payment direction is wrong (regression guard)', () => {
    // If the old bug were present, Bob paying Alice would make Bob's balance
    ...
    // After Bob pays Alice $30 the debt should go to 0, not -60
...
describe('computeSettlements', () => {
  ...
  it('produces one settlement when one debtor owes one creditor', () => {
    const s = computeSettlements({ alice: 60, bob: -30, carol: -30 });
    // bob → alice 30, carol → alice 30  (or any equivalent)
  ...
  it('settles correctly after a partial payment reduces one debt', () => {
    ...
    // alice=+50, bob=-20, carol=-30 → bob→alice $20, carol→alice $30
    const toAlice = s.filter(t => t.to === 'alice');
    const fromBob   = toAlice.find(t => t.from === 'bob');
    const fromCarol = toAlice.find(t => t.from === 'carol');
