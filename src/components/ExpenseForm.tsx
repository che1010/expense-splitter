...
export default function ExpenseForm({ people, onSave, onCancel }: Props) {
  ...
  const taxRate = taxPreset === -1 ? parseFloat(customTax) || 0 : taxPreset;
  ...
  // Re-init splits when people selection or split type changes
  useEffect(() => {
    const init: Record<string, string> = {};
    selectedPeople.forEach(id => { init[id] = ''; });
    setCustomSplits(init);

    const evenPct = selectedPeople.length > 0
      ? (100 / selectedPeople.length).toFixed(2)
      : '0';
    const pctInit: Record<string, string> = {};
    selectedPeople.forEach(id => { pctInit[id] = evenPct; });
    setPctSplits(pctInit);
  }, [selectedPeople, splitType]);
  ...
  const computeSplits = (): Split[] => {
    ...
    if (splitType === 'even') {
      ...
      return selectedPeople.map((id, i) => ({
        ...
        percentage: Math.round((100 / selectedPeople.length) * 100) / 100,
      }));
    }
    ...
    return selectedPeople.map(id => {
      ...
      return {
        personId: id,
        amount: amt,
        percentage: total > 0 ? Math.round((amt / total) * 10000) / 100 : 0,
      };
    });
  };
  ...
  const isValid =
    ...
    (splitType === 'even' || (splitType === 'percentage' ? pctDiff < 0.1 : splitDiff < 0.01));
  ...
  return (
    <div className="card space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-bold text-gray-800">New Expense</h2>
      ...
      <div>
        ...
        {taxRate > 0 && (
          <p className="text-xs text-gray-500 mt-1">
            Tax: ${taxAmount.toFixed(2)} ({taxRate}% of ${subtotalNum.toFixed(2)})
          </p>
        )}
      </div>
      ...
      <div>
        ...
        {tipRate > 0 && (
          <p className="text-xs text-gray-500 mt-1">
            Tip: ${tipAmount.toFixed(2)} ({tipRate}% of ${subtotalNum.toFixed(2)})
          </p>
        )}
      </div>
      ...
      {subtotalNum > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-sm space-y-1">
          ...
          {tipAmount > 0 && (
            <div className="flex justify-between text-gray-600">
              <span>Tip ({tipRate}%)</span><span>${tipAmount.toFixed(2)}</span>
            </div>
          )}
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {(['even', 'percentage', 'custom'] as SplitType[]).map(type => (
            <button
              key={type}
            ...
            >
              {type === 'even' ? 'Even' : type === 'percentage' ? 'By %' : 'Custom $'}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Split inputs */}
      {splitType !== 'even' && selectedPeople.length > 0 && (
            ...
            return (
              <div key={id} className="flex items-center gap-3">
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    className={`input ${splitType === 'custom' ? 'pl-7' : ''}`}
                    type="number"
                    min="0"
                    ...
                    onChange={e => {
                      if (splitType === 'percentage') {
                        setPctSplits(prev => ({ ...prev, [id]: e.target.value }));
                      } else {
